...
public final class NetroBook {
    ...
    private static String getFullGuideText() {
        return ""
            ...
            + C_BODY + "Netro is a Minecraft (Bukkit/Spigot 1.21–1.21.11) plugin for rail networks: stations, transfer nodes, terminals, and cart routing.\n\n"
            + "It lets you: define stations with 2D hierarchical addresses (e.g. OV:E2:N3:01:02:05 Overworld, NE:W1:S2:02:03:01 Nether); attach transfer nodes and terminals to stations; use detector rails and copper bulbs with signs to trigger rules; route carts by destination with on-the-fly shortest path.\n\n"
            ...
            + "Rules: Stored per node. Each rule has a trigger (ENTERING, CLEARING, DETECTED), optional destination condition, and an action (set speed, SEND_ON/OFF, set rail state)." + C_RESET + "\n\n"
            + "---\n\n"
            ...
            + "2) READY: Cart passes the READY detector. Plugin: marks cart held (detection-based; 5s timeout after cart leaves unless CLEAR fires); if destination is this terminal, clears destination; if this cart's turn and dispatch allowed, turns RELEASE ON; brief center hold.\n\n"
            + "3) CLEAR: Cart passes in clearing direction. Plugin: sets held false, cancels 5s timeout; clears held state; turns OFF RELEASE; may turn RELEASE ON for another cart if held.\n\n"
            + "READY = cart held at terminal. RELEASE = power release mechanism. CLEAR = cart left, held false, update state." + C_RESET + "\n\n"
            + "---\n\n"
            ...
            + C_BODY + "Open Rules: Sneak + right-click a [Transfer] or [Terminal] sign. Or: Railroad Controller → right-click [Station] → click node (Node Options) → Open rules. Relocate: from Rules slot 52 or Node Options; click block to place above (one-click). Block above crosshair is highlighted; for Set rail state, the rail you look at is highlighted. Holding the Railroad Controller shows region boundaries (mainnet red, cluster cyan, localnet green) within 80 blocks.\n\n"
            + "Layout: Slots 0–44 = rules. Slot 46 = Pair (transfer only). Slot 49 = Create rule. Slot 52 = Relocate (move detector/controller; click block to place above). Slot 53 = Close.\n\n"
            + "Create rule: Trigger → Destination → Action.\n\n"
            + "Trigger: When cart enters (ENTERING = ENTRY only; READY does not apply rules); when cart clears (CLEARING); when cart detected (DETECTED = ENTRY or CLEAR).\n\n"
            + "Destination: Going to / not going to a destination; any destination; not any destination.\n\n"
            + "Action: Turn bulb ON/OFF (SEND_ON/SEND_OFF); Set rail state (right-click normal rail with Railroad Controller; Cancel in shape picker = back to Action menu); Set cart speed (cruise)." + C_RESET + "\n\n"
            + "---\n\n"
            ...
            + "Command: /netro setdestination <address|name|Station:Terminal>. Examples: OV:E2:N3:01:02:05, Snowy2, Snowy2:0.\n\n"
            ...
            + "CLEAR = cart left the slot; held set false, timeout cancelled; next cart may get RELEASE.\n\n"
