...
public class ControllerRepository {
    ...
    public void insert(Controller c) {
        database.withConnection(conn -> {
            try (PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO controllers (id, node_id, junction_id, world, x, y, z, sign_facing, rule_1_role, rule_1_direction, rule_2_role, rule_2_direction, rule_3_role, rule_3_direction, rule_4_role, rule_4_direction, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")) {
                ps.setString(1, c.getId());
                ps.setString(2, c.getNodeId());
                ps.setString(3, c.getJunctionId());
    ...
    /** Find controllers for this junction that match role and direction (direction null = any). */
    public List<Controller> findByJunctionAndRule(String junctionId, String role, String direction) {
        return database.withConnection(conn -> {
            try (PreparedStatement ps = conn.prepareStatement(
                "SELECT id, node_id, junction_id, world, x, y, z, sign_facing, rule_1_role, rule_1_direction, rule_2_role, rule_2_direction, rule_3_role, rule_3_direction, rule_4_role, rule_4_direction FROM controllers WHERE junction_id = ? AND ( (rule_1_role = ? AND (rule_1_direction IS NULL OR rule_1_direction = ?)) OR (rule_2_role = ? AND (rule_2_direction IS NULL OR rule_2_direction = ?)) OR (rule_3_role = ? AND (rule_3_direction IS NULL OR rule_3_direction = ?)) OR (rule_4_role = ? AND (rule_4_direction IS NULL OR rule_4_direction = ?)) )")) {
    ...
    public Optional<Controller> findByBlock(String world, int x, int y, int z) {
        return database.withConnection(conn -> {
            try (PreparedStatement ps = conn.prepareStatement(
                "SELECT id, node_id, junction_id, world, x, y, z, sign_facing, rule_1_role, rule_1_direction, rule_2_role, rule_2_direction, rule_3_role, rule_3_direction, rule_4_role, rule_4_direction FROM controllers WHERE world = ? AND x = ? AND y = ? AND z = ?")) {
    ...
    public List<Controller> findByNodeId(String nodeId) {
        return database.withConnection(conn -> {
            try (PreparedStatement ps = conn.prepareStatement(
                "SELECT id, node_id, junction_id, world, x, y, z, sign_facing, rule_1_role, rule_1_direction, rule_2_role, rule_2_direction, rule_3_role, rule_3_direction, rule_4_role, rule_4_direction FROM controllers WHERE node_id = ?")) {
                ps.setString(1, nodeId);
                try (ResultSet rs = ps.executeQuery()) {
                    List<Controller> list = new ArrayList<>();
                    while (rs.next()) list.add(rowToController(rs));
